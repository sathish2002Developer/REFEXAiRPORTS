  ...
  return (
    <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12 bg-gray-50 rounded-3xl overflow-hidden w-full">
      ...
      <div className="w-full lg:w-1/2 shrink-0">
        <div className="relative overflow-hidden h-64 sm:h-72 md:h-80 lg:h-[480px]">
          <img
...
export default function Stories({
  ...
  const stories = data?.items?.length ? data.items : defaultStories;
  ...
  const goTo = useCallback((targetIndex: number) => {
    ...
    // After 400ms: remove exit card (it has zoomed out)
    const t1 = setTimeout(() => {
      setExitIndex(null);
    }, 400);

    // After 900ms: transition complete, make enter card the active card
    const t2 = setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-6">
        {/* Card viewport — overflow hidden prevents zoom from causing scrollbars */}
        <div className="relative overflow-hidden rounded-3xl bg-gray-50 min-h-[600px] md:min-h-[680px] lg:min-h-[480px]">
          {phase === 'transitioning' ? (
            <>
              {/* Outgoing card — on top, zooms out */}
              {exitIndex !== null && (
                <div
                  key={`exit-${exitIndex}`}
                  className="absolute inset-0 z-10 animate-story-exit"
                >
                  <StoryCard story={stories[exitIndex]} isEntering={false} />
                </div>
              )}

              {/* Incoming card — behind, zooms in from background */}
              {enterIndex !== null && (
                <div
                  key={`enter-${enterIndex}`}
                  className="absolute inset-0 z-[5] animate-story-enter"
                >
                  <StoryCard story={stories[enterIndex]} isEntering />
                </div>
              )}
            </>
          ) : (
            /* Idle: single static card */
            <div key={`active-${activeIndex}`}>
              <StoryCard story={stories[activeIndex]} isEntering={false} />
            </div>
          )}
        </div>
