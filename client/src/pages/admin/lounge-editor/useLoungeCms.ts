...
export function useLoungeCms() {
  ...
  const addAmenity = () => { setDraft((p) => ({ ...p, amenities: [...p.amenities, { icon: 'ri-star-line', title: '', description: '' }] })); setSaved(false); };
  const removeAmenity = (i: number) => { setDraft((p) => ({ ...p, amenities: p.amenities.filter((_, n) => n !== i) })); setSaved(false); };
  ...
  const addAccess = () => { setDraft((p) => ({ ...p, accessOptions: [...p.accessOptions, { type: '', desc: '', icon: 'ri-user-line' }] })); setSaved(false); };
  const removeAccess = (i: number) => { setDraft((p) => ({ ...p, accessOptions: p.accessOptions.filter((_, n) => n !== i) })); setSaved(false); };

  const handleSave = async () => {
    setSaving(true); setError('');
    try {
      const data = await cmsAdminPatch<Record<string, any>>(`lounge/${airportId}`, { ...currentAirport, ...draft });
      setDraft(toLoungeDraft(data)); setSaved(true); setTimeout(() => setSaved(false), 3000);
